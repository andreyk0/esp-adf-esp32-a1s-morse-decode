...
int N = N_SAMPLES;
// Input test array
__attribute__((aligned(16))) float d[N_SAMPLES];
// output array
__attribute__((aligned(16))) float y[N_SAMPLES];
...
void ShowIIRfilter(float freq, float qFactor) {
  ...
  ESP_LOGI(TAG, "Impulse response of IIR filter with F=%f, qFactor=%f", freq,
           qFactor);
  ...
  for (int i = 0; i < N / 2; i++) {
    y_cf[i] = 10 * log10f((y_cf[i * 2 + 0] * y_cf[i * 2 + 0] +
                           y_cf[i * 2 + 1] * y_cf[i * 2 + 1]) /
                          N);
  }
